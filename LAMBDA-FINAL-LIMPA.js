...
return { statusCode: 200, headers, body: JSON.stringify({ credits: parseInt(credits), baeCredits: parseInt(baeCredits), trmvCredits: parseInt(trmvCredits), name }) };
}

// POST /use-credit
if (path === "/use-credit" && method === "POST") {
const tipo = body.tipo || "tecfe";
const creditField = tipo === "bae" ? "baeCredits" : tipo === "trmv" ? "trmvCredits" : "credits";
const res = await db.send(new GetItemCommand({TableName: "tecfe-users",Key: { email: { S: email } }}));
...
if (!res.Item) {const item = { email: { S: targetEmail }, credits: { N: "0" }, baeCredits: { N: "0" }, name: { S: "" } };item[creditField] = { N: String(amount) };await db.send(new PutItemCommand({ TableName: "tecfe-users", Item: item }));} else {await db.send(new UpdateItemCommand({TableName: "tecfe-users",Key: { email: { S: targetEmail } },UpdateExpression: "SET #cf = if_not_exists(#cf, :zero) + :amt",ExpressionAttributeNames: { "#cf": creditField },ExpressionAttributeValues: { ":amt": { N: String(amount) }, ":zero": { N: "0" } }}));}
...
const aiKey = "AIzaSyA_Xxm8ObINuPtn_23Xfo5pAUWSNzfhGvI";
